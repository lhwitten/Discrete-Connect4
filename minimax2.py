...
def evaluate_window(window, player):
    ...
    if window.count(player) == 2 and window.count(' ') == 2 and ' ' in window:
        score += 100  # Encourage creating a winning opportunity
...
def minimax(board, depth, maximizing_player, alpha, beta):
    ...
    #if we have recursively made depth 0
    ...
    if maximizing_player:
        ...
        for col in valid_locations:
            ...
            new_score = disc_factor * minimax(temp_board, depth-1, False, alpha, beta)[1]
            if new_score > value:
                value = new_score
                column = col
        ...
        for col in valid_locations:
            ...
            #reduce the depth by 1, switch the player (True/false for maximizing)
            new_score = disc_factor * minimax(temp_board, depth-1, True, alpha, beta)[1]
            if new_score < value:
                value = new_score
                column = col
            beta = min(beta, value)
...
def play_connect4():
    ...
    while not game_over:
        if turn % 2 == 0:
            col, minimax_score = minimax(board, 6, True, -np.Inf, np.Inf)
            if is_valid_move(board, col):
